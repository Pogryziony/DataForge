...
export function useWorkbench() {
  ...
  const connect = useCallback(() => {
    const instance = new Worker(new URL('../workers/generation.worker.ts', import.meta.url), {
      type: 'module',
    });
      ...
      if (message.type === 'progress') setProgress((message.completed / message.total) * 100);
        ...
        if (message.manifest)
          void database.history
            ...
            .catch(() =>
              setNotice(
                'History could not be saved. Export the manifest to retain this configuration.',
              ),
            );
        ...
        downloadArtifact(message.artifact);
        setBusy(false);
      }
    };
    instance.onerror = () => {
      setError('Background worker failed. Cancel and retry; no partial result is marked complete.');
      setBusy(false);
    };
  ...
  const cancel = () => {
    ...
    setProgress(0);
    clear();
    setReport('Cancelled. Partial results were discarded.');
  };
  return {
    rows,
    count,
