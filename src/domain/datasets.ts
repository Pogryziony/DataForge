...
export function datasetDependencies(fields: FieldDefinition[]): string[] {
  return fields.flatMap((field) => [
    ...
    ...datasetDependencies(field.fields ?? []),
    ...datasetDependencies(field.item ? [field.item] : []),
  ]);
}
export function generateDatasets(
  definitions: DatasetDefinition[],
  base: Omit<GenerationConfig, 'schema' | 'count' | 'datasets'>,
  ...
  if (
    ...
    definitions.some(
      (definition) =>
        ...
        definition.count < 1 ||
        !/^(?!__proto__$|constructor$|prototype$)[A-Za-z_][A-Za-z0-9_]*$/.test(definition.name),
    )
  )
    throw new DomainError('DATASET_CONFIG', 'Provide named datasets with positive integer counts');
  if (definitions.reduce((count, dataset) => count + dataset.count, 0) > 100000)
    throw new DomainError('DATASET_LIMIT', 'Combined dataset count cannot exceed 100000');
  const indexed = new Map(definitions.map((dataset) => [dataset.name, dataset]));
  if (indexed.size !== definitions.length)
    throw new DomainError('DATASET_NAME', 'Dataset names must be unique');
  const result: Record<string, DataRecord[]> = {};
  const active = new Set<string>();
  function visit(name: string) {
    if (Object.hasOwn(result, name)) return;
    if (['__proto__', 'constructor', 'prototype'].includes(name))
      throw new DomainError('DATASET_NAME', 'Unsafe dataset name');
    const definition = indexed.get(name);
    if (!definition)
      throw new DomainError('DATASET_MISSING', 'Referenced dataset does not exist', name);
    if (active.has(name))
      throw new DomainError('DATASET_CYCLE', 'Cyclic dataset dependencies', name);
    active.add(name);
    ...
    const groups = (definition.uniqueTogether ?? []).map((keys) => {
      if (
        ...
        keys.some((key) => !schema.fields.some((field) => field.name === key))
      )
        throw new DomainError(
          'COMPOSITE_FIELDS',
          'Composite constraints need distinct existing top-level fields',
          name,
        );
    ...
    for (const row of rows) {
      ...
      if (issues.length)
        throw new DomainError('DATASET_VALIDATION', issues[0].message, `${name}.${issues[0].path}`);
    }
    result[name] = rows;
    active.delete(name);
  }
