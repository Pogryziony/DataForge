...
export function randomDate(random: SeededRandom, min = '1950-01-01', max = '2005-12-31'): string {
  const day = random.integer(
    parseDate(min).getTime() / 86400000,
    parseDate(max).getTime() / 86400000,
  );
...
export function ageOn(birthDate: string, referenceDate: string): number {
  ...
  if (birthDate > referenceDate)
    throw new DomainError('FUTURE_BIRTH', 'Birth date is after reference date');
  return (
    reference.year -
    birth.year -
    Number(
...
export function addDays(value: string, days: number): string {
  if (!Number.isSafeInteger(days))
    throw new DomainError('INVALID_OFFSET', 'Date offset must be an integer');
  const result = new Date(parseDate(value).getTime() + days * 86400000);
  if (!Number.isFinite(result.getTime()))
    throw new DomainError('INVALID_DATE', 'Date offset exceeds supported range');
...
export function zonedTimestamp(date: Date, timeZone: string): string {
  ...
  try {
    ...
    throw new DomainError('TIME_ZONE', 'Provide a supported IANA time zone');
  }
