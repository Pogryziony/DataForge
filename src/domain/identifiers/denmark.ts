...
export function decodeCpr(input: string): {
  birthDate: string;
  sex: EncodedSex;
  modulus11: boolean;
} {
  if (!/^(?:\d{10}|\d{6}-\d{4})$/.test(input))
    throw new DomainError('CPR_FORMAT', 'Expected DDMMYYXXXX or DDMMYY-XXXX');
  ...
  return {
    ...
    modulus11:
      [...value].reduce((sum, digit, i) => sum + Number(digit) * CPR_WEIGHTS[i], 0) % 11 === 0,
  };
}
export function validateCpr(
  input: string,
  profile: CprProfile = 'standard',
  officialPool: string[] = [],
): boolean {
  try {
    ...
    if (profile === 'official-test-pool')
      return officialPool.some((value) => value.replace('-', '') === input.replace('-', ''));
    return true;
  } catch {
    return false;
  }
}
export function generateCpr(
  random: SeededRandom,
  birthDate: string,
  sex: EncodedSex,
  profile: CprProfile = 'standard',
  ...
  if (year < 1858 || year > 2057)
    throw new DomainError('CPR_YEAR_RANGE', 'CPR supports years 1858–2057');
  ...
  if (profile === 'official-test-pool') {
    ...
    const seventhDigits = Array.from({ length: 10 }, (_, i) => i).filter(
      (digit) => cprYear(year % 100, digit) === year,
    );
    for (let attempt = 0; attempt < 2000; attempt++) {
      const candidate =
        prefix +
        random.pick(seventhDigits) +
        random.digits(2) +
        (random.integer(0, 4) * 2 + Number(sex === 'male'));
      if (validateCpr(candidate, profile))
        return formatted ? candidate.slice(0, 6) + '-' + candidate.slice(6) : candidate;
    }
    throw new DomainError(
      'CPR_EXHAUSTED',
      'Unable to satisfy the CPR profile within the retry limit',
    );
  }
...
export function validateCvr(value: string): boolean {
  return (
    /^[1-9]\d{7}$/.test(value) &&
    [...value].reduce((sum, char, i) => sum + Number(char) * CVR_WEIGHTS[i], 0) % 11 === 0
  );
}
export function generateCvr(random: SeededRandom): string {
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    const remainder =
      [...body].reduce((sum, char, i) => sum + Number(char) * CVR_WEIGHTS[i], 0) % 11;
...
export const generatePNumber = (random: SeededRandom): string =>
  random.integer(1, 9) + random.digits(9);
