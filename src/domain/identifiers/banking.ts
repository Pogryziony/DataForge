...
export function ibanFromBban(country: IbanCountry, bban: string): string {
  const expected = country === 'GB' ? /^[A-Z]{4}\d{14}$/ : new RegExp(`^\\d{${lengths[country] - 4}}$`);
...
export function generateIban(random: SeededRandom, country: IbanCountry): string {
  if (!(country in lengths)) throw new DomainError('IBAN_COUNTRY', 'IBAN is supported for PL, DK, DE and GB; not US');
  let bban = country === 'GB' ? 'TEST' + random.digits(14) : random.digits(lengths[country] - 4);
  if (country === 'PL') {
    const prefix = random.digits(7);
    const sum = [...prefix].reduce((total, digit, i) => total + Number(digit) * [3, 9, 7, 1, 3, 9, 7][i], 0);
...
export function validateIban(value: string): boolean {
  ...
  if (value.length !== lengths[country] || Number(value.slice(2, 4)) < 2 || Number(value.slice(2, 4)) > 98) return false;
