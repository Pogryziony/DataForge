...
import { applyRule, numberOption, readPath, stringOption, type GeneratorContext, type GeneratorDefinition } from './generators';
...
import { decodePesel, validateNip, validatePesel, validatePolishId, validateRegon } from './identifiers/poland';
import { validateIban } from './identifiers/banking';
import type { CprProfile, DataRecord, FieldDefinition, GenerationConfig, GenerationManifest, JsonValue } from './types';
...
export function orderFields(fields: FieldDefinition[]): FieldDefinition[] {
  ...
  const done = new Set<string>(), active = new Set<string>(), ordered: FieldDefinition[] = [];
  function visit(field: FieldDefinition) {
    if (done.has(field.name)) return;
    if (active.has(field.name)) throw new DomainError('DEPENDENCY_CYCLE', 'Cyclic field dependencies', field.name);
    active.add(field.name);
    const dependencies = [
      ...(field.rule?.fields ?? []),
      ...(field.condition ? [field.condition.field] : []),
      stringOption(field, 'birthDateField'), stringOption(field, 'sexField'),
    ].filter(Boolean);
    for (const path of dependencies) {
      const dependency = indexed.get(path.split('.')[0]);
      if (!dependency) throw new DomainError('MISSING_DEPENDENCY', 'Referenced field is not in this object', path);
      visit(dependency);
    }
...
export function validateConfiguration(config: GenerationConfig, registry: Map<string, GeneratorDefinition>): void {
  if (!Number.isInteger(config.count) || config.count < 1 || config.count > 100000) throw new DomainError('RECORD_LIMIT', 'Record count must be 1–100000');
  if (typeof config.seed !== 'string' || !config.seed.length || config.seed.length > 1000) throw new DomainError('SEED', 'Provide a seed of 1–1000 characters');
  if (!['pl', 'da', 'de', 'en_GB', 'en_US'].includes(config.locale)) throw new DomainError('LOCALE', 'Unsupported locale');
  parseDate(config.referenceDate);
  if (!config.schema || !Array.isArray(config.schema.fields) || !config.schema.fields.length) throw new DomainError('EMPTY_SCHEMA', 'Add at least one field');
  let fieldCount = 0;
  function check(fields: FieldDefinition[], depth: number): void {
    ...
    for (const field of fields) {
      if (++fieldCount > 200) throw new DomainError('FIELD_LIMIT', 'Maximum schema size is 200 fields');
      if (!field || !safeName.test(field.name) || names.has(field.name)) throw new DomainError('FIELD_NAME', 'Use unique identifier-style field names', field?.name);
      if (!field.id || ids.has(field.id)) throw new DomainError('FIELD_ID', 'Field IDs must be unique and non-empty', field.name);
      names.add(field.name); ids.add(field.id);
      ...
      if (field.generator === 'object') check(field.fields ?? [], depth + 1);
        ...
        const min = numberOption(field, 'minItems', 1), max = numberOption(field, 'maxItems', 3);
        if (!Number.isInteger(min) || !Number.isInteger(max) || min < 0 || max > 1000 || min > max || !field.item) throw new DomainError('ARRAY_LIMIT', 'Arrays need an item and a range within 0–1000', field.name);
        check([field.item], depth + 1);
      } else if (!registry.has(field.generator) && !field.rule) throw new DomainError('GENERATOR', 'Unknown generator', field.name);
      if (field.rule && !['copy', 'email', 'age', 'addDays', 'multiply', 'template', 'foreignKey'].includes(field.rule.operation)) throw new DomainError('RULE_OPERATION', 'Unknown rule', field.name);
    }
    orderFields(fields);
  }
...
export class GenerationSession {
  ...
  private nextIndex = 0;
  constructor(public readonly config: GenerationConfig, private readonly registry: Map<string, GeneratorDefinition>) {
  ...
  private object(fields: FieldDefinition[], path: string, index: number, recordAttempt = 0): DataRecord {
    ...
    for (const field of ordered) {
      ...
      if (field.condition && JSON.stringify(readPath(row, field.condition.field)) !== JSON.stringify(field.condition.equals)) continue;
      ...
      if (roll < missingAndNull) { row[field.name] = null; continue; }
      if (roll < missingAndNull + (field.emptyRate ?? 0)) { row[field.name] = ''; continue; }
      ...
      for (let attempt = 0; attempt < 1000; attempt++) {
        ...
        if (!field.unique || !occupied.has(key)) {
          ...
          if (field.unique) { occupied.add(key); this.unique.set(fieldPath, occupied); }
          accepted = true;
          break;
        }
      }
      if (!accepted) throw new DomainError('UNIQUE_EXHAUSTED', 'Unique values exhausted or retry limit reached; relax constraints or reduce count', field.name);
    }
...
export function validateRecord(row: DataRecord, config: GenerationConfig): ValidationIssue[] {
  ...
  function inspect(record: DataRecord, fields: FieldDefinition[], prefix: string) {
    for (const field of fields) {
      const path = prefix + field.name, value = record[field.name];
      if (field.condition && JSON.stringify(readPath(record, field.condition.field)) !== JSON.stringify(field.condition.equals)) continue;
      if (value === undefined) { if (field.required !== false) issues.push(issue('REQUIRED', 'Required field is absent', path)); continue; }
      ...
      if (field.generator === 'object') {
        ...
        if (!Array.isArray(value)) fail('TYPE', 'Expected array');
          ...
          if (value.length < numberOption(field, 'minItems', 1) || value.length > numberOption(field, 'maxItems', 3)) fail('RANGE', 'Array size outside range');
          value.forEach((item, index) => inspect({ [field.item!.name]: item }, [field.item!], `${path}.${index}.`));
        }
      } else {
        const numeric = ['integer', 'sequence', 'decimal', 'amount', 'age'].includes(field.generator);
        if (numeric && typeof value !== 'number') fail('TYPE', 'Expected number');
        if (field.generator === 'boolean' && typeof value !== 'boolean') fail('TYPE', 'Expected boolean');
        if (numeric && typeof value === 'number') {
          if (field.options?.min !== undefined && value < Number(field.options.min) || field.options?.max !== undefined && value > Number(field.options.max)) fail('RANGE', 'Number outside configured range');
          if (['integer', 'amount', 'age'].includes(field.generator) && !Number.isInteger(value)) fail('TYPE', 'Expected integer');
        }
        const validators: Record<string, (value: string) => boolean> = {
          pesel: validatePesel, nip: validateNip, regon: validateRegon, polishId: validatePolishId,
          iban: validateIban, nrb: value => validateIban('PL' + value), cvr: validateCvr, pNumber: validatePNumber,
          vatDk: value => /^DK\d{8}$/.test(value) && validateCvr(value.slice(2)),
          cpr: value => validateCpr(value, stringOption(field, 'profile', 'standard') as CprProfile, config.pools?.filter(pool => pool.kind === 'cpr' && (!stringOption(field, 'sourceId') || pool.source.id === stringOption(field, 'sourceId'))).flatMap(pool => pool.records.map(record => String(record.cpr)))),
          uuid: value => /^[\da-f]{8}-[\da-f]{4}-4[\da-f]{3}-[89ab][\da-f]{3}-[\da-f]{12}$/i.test(value),
          email: value => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value),
          date: value => { try { parseDate(value); return true; } catch { return false; } },
          birthDate: value => { try { parseDate(value); return true; } catch { return false; } },
        };
        if (validators[field.generator] && (typeof value !== 'string' || !validators[field.generator](value))) fail('FORMAT', 'Value does not satisfy generator profile');
        if (field.generator === 'text') {
          if (typeof value !== 'string') fail('TYPE', 'Expected text');
          else {
            const length = [...value].length - [...stringOption(field, 'prefix')].length - [...stringOption(field, 'suffix')].length;
            if (length < numberOption(field, 'minLength', 12) || length > numberOption(field, 'maxLength', 24)) fail('LENGTH', 'Text length outside range');
          }
        }
        if (field.generator === 'enum' && !Array.isArray(field.options?.values)) fail('ENUM', 'Missing enum values');
        else if (field.generator === 'enum' && Array.isArray(field.options?.values) && !field.options.values.some(candidate => JSON.stringify(candidate) === JSON.stringify(value))) fail('ENUM', 'Not an allowed value');
        if (field.generator === 'constant' && JSON.stringify(value) !== JSON.stringify(field.options?.value ?? null)) fail('CONST', 'Constant differs');
        if (typeof value === 'string' && ['pesel', 'cpr'].includes(field.generator) && validators[field.generator](value)) {
          const decoded = field.generator === 'pesel' ? decodePesel(value) : decodeCpr(value);
          const birthField = stringOption(field, 'birthDateField'), sexField = stringOption(field, 'sexField');
          if (birthField && readPath(record, birthField) !== decoded.birthDate) fail('DATE_RELATION', 'Birth date differs from identifier');
          if (sexField && readPath(record, sexField) !== decoded.sex) fail('SEX_RELATION', 'Encoded sex differs');
        }
      }
      if (field.rule && field.rule.operation !== 'foreignKey') {
        try {
          const expected = applyRule({ random: new SeededRandom(config.seed), row: record, index: 0, config, field });
          if (JSON.stringify(expected) !== JSON.stringify(value)) fail('RELATION', 'Derived value does not match its dependencies');
        } catch { fail('RELATION', 'Dependency cannot be evaluated'); }
      } else if (field.rule?.operation === 'foreignKey') {
        const rule = field.rule;
        if (!config.datasets?.[rule.dataset ?? '']?.some(parent => JSON.stringify(readPath(parent, rule.targetField ?? 'id')) === JSON.stringify(value))) fail('FOREIGN_KEY', 'Referenced record does not exist');
      }
    }
  }
