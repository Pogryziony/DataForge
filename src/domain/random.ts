...
export class SeededRandom {
  ...
  constructor(public readonly seed: string) { this.state = hashSeed(seed); }
  next(): number {
  ...
  integer(min: number, max: number): number {
    if (!Number.isSafeInteger(min) || !Number.isSafeInteger(max) || min > max || max - min > Number.MAX_SAFE_INTEGER) {
      throw new DomainError('INVALID_RANGE', 'Expected an ordered safe integer range');
    }
    return min + Math.floor(this.next() * (max - min + 1));
  }
  pick<T>(values: readonly T[]): T {
    if (!values.length) throw new DomainError('EMPTY_POOL', 'No candidate values match these constraints');
    return values[this.integer(0, values.length - 1)];
  }
  digits(length: number): string {
    if (!Number.isInteger(length) || length < 0 || length > 100_000) throw new DomainError('INVALID_LENGTH', 'Length must be 0–100000');
