...
export const stringOption = (field: FieldDefinition, key: string, fallback = ''): string =>
  typeof field.options?.[key] === 'string' ? String(field.options[key]) : fallback;
export const numberOption = (field: FieldDefinition, key: string, fallback: number): number =>
  typeof field.options?.[key] === 'number' ? Number(field.options[key]) : fallback;
...
const contextDate = ({ field, row, random }: GeneratorContext): string => {
  ...
  if (linked) {
    ...
    if (typeof value !== 'string')
      throw new DomainError('BIRTH_DATE_REQUIRED', 'Linked birth date is missing', linked);
    return value;
  }
  return (
    stringOption(field, 'birthDate') ||
    randomDate(
      random,
      stringOption(field, 'minDate', '1950-01-01'),
      stringOption(field, 'maxDate', '2005-12-31'),
    )
  );
};
const contextSex = ({ field, row, random }: GeneratorContext): EncodedSex => {
  ...
  if (sex !== undefined && sex !== 'male' && sex !== 'female')
    throw new DomainError('ENCODED_SEX', 'Expected male or female for the identifier');
...
export function createGeneratorRegistry(text: TextProvider): Map<string, GeneratorDefinition> {
  ...
  const add = (
    ...
    generate: GeneratorDefinition['generate'],
  ) => definitions.push({ id, label, category, generate });
  ...
  add('integer', 'Integer', 'General', (c) =>
    c.random.integer(numberOption(c.field, 'min', 0), numberOption(c.field, 'max', 1000)),
  );
  add('decimal', 'Decimal', 'General', (c) => {
    const precision = numberOption(c.field, 'precision', 2);
    if (!Number.isInteger(precision) || precision < 0 || precision > 6)
      throw new DomainError('PRECISION', 'Precision must be 0–6');
    const factor = 10 ** precision;
    return (
      c.random.integer(
        Math.ceil(numberOption(c.field, 'min', 0) * factor),
        Math.floor(numberOption(c.field, 'max', 1000) * factor),
      ) / factor
    );
  });
  add('boolean', 'Boolean', 'General', (c) => {
    ...
    if (!Number.isFinite(rate) || rate < 0 || rate > 1)
      throw new DomainError('BOOLEAN_RATE', 'trueRate must be between 0 and 1');
  ...
  add(
    ...
    'Sequence',
    'General',
    (c) => numberOption(c.field, 'start', 1) + c.index * numberOption(c.field, 'step', 1),
  );
  add('enum', 'Weighted dictionary', 'General', (c) => {
    const values = c.field.options?.values;
    if (!Array.isArray(values) || !values.length)
      throw new DomainError('ENUM_VALUES', 'Provide a non-empty values array');
    const excluded = c.field.options?.excluded;
    const candidates = values
      .map((value, i) => ({
        value,
        weight: Array.isArray(c.field.options?.weights) ? Number(c.field.options.weights[i]) : 1,
      }))
      .filter(
        (entry) =>
          !Array.isArray(excluded) ||
          !excluded.some((value) => JSON.stringify(value) === JSON.stringify(entry.value)),
      );
    if (candidates.some((entry) => !Number.isFinite(entry.weight) || entry.weight < 0))
      throw new DomainError('WEIGHTS', 'Weights must be finite and non-negative');
  ...
  add('text', 'Text', 'General', (c) => {
    ...
    const length = c.random.integer(
      numberOption(c.field, 'minLength', 12),
      numberOption(c.field, 'maxLength', 24),
    );
    if (length > 100_000) throw new DomainError('TEXT_LIMIT', 'Text limit is 100000 characters');
    return (
      stringOption(c.field, 'prefix') +
      Array.from({ length }, () => c.random.pick(alphabet)).join('') +
      stringOption(c.field, 'suffix')
    );
  });
  add('firstName', 'First name', 'Personal', (c) =>
    text.firstName(locale(c), textSeed(c), contextSex(c)),
  );
  ...
  add(
    ...
    'Personal',
    (c) =>
      `${text.firstName(locale(c), textSeed(c), contextSex(c))} ${text.lastName(locale(c), textSeed(c))}`,
  );
  ...
  add('phone', 'Phone (synthetic)', 'Personal', (c) => {
    ...
    return (
      (c.field.options?.international === false ? '' : prefix) +
      c.random.integer(2, 9) +
      c.random.digits(length - 1)
    );
  });
  add('date', 'Date', 'General', (c) =>
    randomDate(
      c.random,
      stringOption(c.field, 'min', '2020-01-01'),
      stringOption(c.field, 'max', c.config.referenceDate),
    ),
  );
  ...
  add(
    ...
    'General',
    (c) =>
      `${String(c.random.integer(0, 23)).padStart(2, '0')}:${String(c.random.integer(0, 59)).padStart(2, '0')}:${String(c.random.integer(0, 59)).padStart(2, '0')}`,
  );
  add('timestamp', 'Timestamp', 'General', (c) => {
    const date = new Date(
      `${randomDate(c.random, stringOption(c.field, 'min', '2020-01-01'), stringOption(c.field, 'max', c.config.referenceDate))}T12:00:00.000Z`,
    );
  ...
  add(
    ...
    'Currency',
    'Banking',
    (c) => ({ pl: 'PLN', da: 'DKK', de: 'EUR', en_GB: 'GBP', en_US: 'USD' })[locale(c)],
  );
  add('amount', 'Amount in minor units', 'Banking', (c) =>
    c.random.integer(numberOption(c.field, 'min', 0), numberOption(c.field, 'max', 100000)),
  );
  add('iban', 'IBAN', 'Banking', (c) =>
    generateIban(
      c.random,
      stringOption(
        c.field,
        'country',
        { pl: 'PL', da: 'DK', de: 'DE', en_GB: 'GB', en_US: 'US' }[locale(c)],
      ) as IbanCountry,
    ),
  );
  add('nrb', 'NRB', 'Poland', (c) => generateIban(c.random, 'PL').slice(2));
  add('testCard', 'Stripe test card', 'Banking', (c) =>
    c.random.pick(['4242424242424242', '4000056655665556', '5555555555554444']),
  );
  add('url', 'Example URL', 'General', (c) => `https://example.com/test/${c.random.digits(6)}`);
  add(
    'ipv4',
    'IPv4 (documentation range)',
    'General',
    (c) => `192.0.2.${c.random.integer(1, 254)}`,
  );
  add(
    'ipv6',
    'IPv6 (documentation range)',
    'General',
    (c) => `2001:db8::${c.random.integer(1, 65535).toString(16)}`,
  );
  add('address', 'Synthetic address', 'Address', (c) =>
    locale(c) === 'da' ? syntheticDanishAddress(c.random) : text.address(locale(c), textSeed(c)),
  );
  ...
  add('regon', 'REGON', 'Poland', (c) => {
    ...
    if (length !== 9 && length !== 14)
      throw new DomainError('REGON_LENGTH', 'REGON length must be 9 or 14');
  ...
  add('cpr', 'CPR', 'Denmark', (c) => {
    ...
    if (!['standard', 'legacy-mod11', 'official-test-pool'].includes(profile))
      throw new DomainError('CPR_PROFILE', 'Unknown CPR profile');
    const sourceId = stringOption(c.field, 'sourceId');
    const pools =
      c.config.pools?.filter(
        (pool) => pool.kind === 'cpr' && (!sourceId || pool.source.id === sourceId),
      ) ?? [];
    const values = pools.flatMap((pool) => pool.records.map((record) => String(record.cpr)));
    if (
      profile === 'official-test-pool' &&
      !stringOption(c.field, 'birthDate') &&
      !stringOption(c.field, 'birthDateField')
    ) {
      const sex = stringOption(c.field, 'sexField')
        ? readPath(c.row, stringOption(c.field, 'sexField'))
        : c.field.options?.sex;
    ...
    return generateCpr(
      ...
      contextSex(c),
      profile,
      Boolean(c.field.options?.formatted),
      values,
    );
  });
  ...
  add('bankDk', 'Danish account components', 'Denmark', (c) => {
    ...
    return {
      registrationNumber: iban.slice(4, 8),
  ...
  add('dar', 'DAR reference address', 'Denmark', (c) =>
    referenceAddress(c.random, c.config.pools ?? [], stringOption(c.field, 'sourceId')),
  );
  add('security', 'Security test string', 'Security', (c) => {
    ...
    if (category === 'long') {
      ...
      if (!Number.isInteger(length) || length < 0 || length > 100000)
        throw new DomainError('TEXT_LIMIT', 'Security samples are limited to 100000 characters');
      return 'A'.repeat(length);
    }
    if (!(category in SECURITY_SAMPLES))
...
export function applyRule(c: GeneratorContext): JsonValue {
  ...
  const values = (rule.fields ?? []).map((path) => {
    ...
    if (value === undefined)
      throw new DomainError('DEPENDENCY_VALUE', 'Dependency has no value', path);
    return value;
  });
  switch (rule.operation) {
    ...
    case 'age':
      return ageOn(String(values[0]), c.config.referenceDate);
    case 'addDays':
      return addDays(String(values[0]), Number(rule.value ?? 1));
    case 'multiply': {
      const result = Number(values[0]) * Number(rule.value ?? 1);
      if (!Number.isFinite(result))
        throw new DomainError('RULE_NUMBER', 'Multiplication requires finite numbers');
      return Math.round(result * 100) / 100;
    }
    case 'template':
      return String(rule.value ?? '').replace(/\{(\d+)\}/g, (_, index: string) =>
        String(values[Number(index)] ?? ''),
      );
    case 'foreignKey': {
      const rows = c.config.datasets?.[rule.dataset ?? ''];
      if (!rows?.length)
        throw new DomainError('FOREIGN_POOL', 'Referenced dataset is missing or empty');
      const value = readPath(c.random.pick(rows), rule.targetField ?? 'id');
      if (value === undefined || value === null)
        throw new DomainError('FOREIGN_KEY', 'Referenced key is absent');
      return value;
    }
    default:
      throw new DomainError('RULE_OPERATION', 'Unknown rule operation');
  }
}
