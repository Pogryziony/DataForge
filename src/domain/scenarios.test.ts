...
it('covers allowed parameter pairs and reports truncation', () => {
  const input = { parameters: { country: ['PL', 'DK'], role: ['user', 'admin'], active: [true, false] }, forbidden: [{ country: 'DK', role: 'admin' }] };
...
it('checks negative mutations rather than claiming arbitrary text is invalid', () => {
  const config: GenerationConfig = { seed: 'a', referenceDate: '2026-09-07', count: 1, locale: 'pl', schema: { id: 's', name: 's', version: 1, fields: [{ id: 'id', name: 'id', generator: 'integer', options: { min: 1, max: 10 } }] } };
...
it('imports supported schema and validates generated data independently', async () => {
  const original = { $schema: 'https://json-schema.org/draft/2020-12/schema', type: 'object', required: ['code', 'amount', 'nested'], additionalProperties: false, properties: {
  ...
  const rows = new GenerationSession({ seed: 'schema', referenceDate: '2026-09-07', count: 20, locale: 'pl', schema }, registry).nextBatch(20);
