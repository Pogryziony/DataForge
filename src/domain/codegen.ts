...
const tsType = (value: JsonValue): string =>
  value === null
    ? 'null'
    : Array.isArray(value)
      ? value.length
        ? `Array<${[...new Set(value.map(tsType))].join(' | ')}>`
        : 'unknown[]'
      : typeof value === 'object'
        ? `{ ${Object.entries(value)
            .map(([key, child]) => `${JSON.stringify(key)}: ${tsType(child)}`)
            .join('; ')} }`
        : typeof value;
    ...
    const fields = Object.entries(row).map(([name, value]) => {
      if (!/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(name) || reserved.has(name))
        throw new DomainError(
          'JAVA_FIELD',
          'Rename fields to valid non-reserved Java identifiers',
          name,
        );
      const type =
        typeof value === 'number'
