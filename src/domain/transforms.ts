    ...
    for (const rule of rules) {
      ...
      switch (rule.operation) {
        ...
        case 'mask': {
          ...
          row[rule.field] = '*'.repeat(Math.max(0, text.length - keep)) + (keep ? text.slice(-keep) : '');
          break;
        }
        case 'pseudonymize': {
          ...
          if (!cache.has(message)) {
            ...
            cache.set(message, 'pseudo_' + [...bytes].map(byte => byte.toString(16).padStart(2, '0')).join(''));
          }
          row[rule.field] = cache.get(message)!;
          break;
        }
        case 'shiftDate': row[rule.field] = addDays(String(value), Number(rule.value ?? 0)); break;
        case 'trim': row[rule.field] = String(value).trim(); break;
        case 'uppercase': row[rule.field] = String(value).toUpperCase(); break;
        case 'lowercase': row[rule.field] = String(value).toLowerCase(); break;
        default: throw new DomainError('TRANSFORM', 'Unsupported transform');
      }
    }
    result.push(row);
  }
