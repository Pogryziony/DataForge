...
it('generates every built-in non-reference template and checks the full record', async () => {
  ...
  for (const template of TEMPLATES.filter(template => template.id !== 'dar-address')) {
    const config = { seed: 'templates', referenceDate: '2026-09-07', locale: 'da' as const, count: 5, schema: template.schema };
...
it('validates DAR relationships without converting synthetics to registry-verified records', () => {
  ...
  const pool: ReferencePool = { kind: 'dar', source: { id: 'test-only', title: 'Synthetic shape fixture, not DAR', url: 'https://example.com/fixture', retrievedAt: '2026-01-01', version: '1', license: 'Test only' }, records: [{ ...address, municipalityCode: '0101', roadCode: '0001' }] };
  expect(() => validateReferencePool(pool)).not.toThrow();
  pool.records.push({ ...pool.records[0], adresseId: new SeededRandom('second').uuid(), houseNumber: '999' });
