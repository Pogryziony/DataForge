  ...
  constructor(name = 'dataforge') {
    ...
    this.version(2)
      .stores({
        ...
        revisions: 'key, templateId, version',
    ...
    try {
      return await this.db.transaction('rw', this.db.templates, this.db.revisions, async () => {
        ...
        const saved = {
          id,
          name: schema.name,
          schema: { ...structuredClone(schema), id, version },
          updatedAt,
        };
        await this.db.templates.put(saved);
        await this.db.revisions.put({
          key: `${id}:${version}`,
          templateId: id,
          version,
          schema: saved.schema,
      ...
      if (error instanceof Error && /quota/i.test(error.name + error.message))
        throw new DomainError(
          'STORAGE_QUOTA',
          'Storage is full. Export your templates before removing stored data',
        );
      throw new DomainError(
        'STORAGE_SAVE',
        'Local storage could not save the template; export it as JSON',
      );
    }
  }
  async revisions(id: string): Promise<TemplateRevision[]> {
    return this.db.revisions.where('templateId').equals(id).sortBy('version');
  }
