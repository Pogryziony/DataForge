...
function locales(fields: FieldDefinition[]): Locale[] {
  return fields.flatMap((field) => [
    ...(field.locale ? [field.locale] : []),
    ...locales(field.fields ?? []),
    ...locales(field.item ? [field.item] : []),
  ]);
}
...
self.onmessage = async (event: MessageEvent<WorkerRequest>) => {
  ...
  try {
    if (request.type === 'export') {
      ...
      send({ type: 'artifact', jobId: request.jobId, artifact });
      return;
    }
    if (request.type === 'code') {
      if (records.length > 1000)
        throw new DomainError(
          'CODE_LIMIT',
          'Code generation is limited to 1000 records; export a fixture file for larger sets',
        );
      const suffix =
        request.target === 'cypress'
          ? 'json'
          : ['java', 'rest-assured'].includes(request.target)
            ? 'java'
            : 'ts';
      send({
        type: 'artifact',
        ...
        artifact: {
          filename:
            request.target === 'java'
              ? 'TestRecord.java'
              : request.target === 'rest-assured'
                ? 'TestPayload.java'
                : `dataforge-${request.target}.${suffix}`,
          mime: 'text/plain',
          contents: generateCode(records, request.target),
        },
      });
      return;
    }
    if (request.type === 'manifest') {
      send({
        ...
        jobId: request.jobId,
        artifact: {
          filename: 'dataforge-manifest.json',
          mime: 'application/json',
          contents: JSON.stringify(
            manifest ?? { note: 'Transformed or pairwise data; no generation manifest' },
            null,
            2,
          ),
        },
    ...
    if (request.type === 'generate') {
      const registry = createGeneratorRegistry(
        await loadTextProvider([request.config.locale, ...locales(request.config.schema.fields)]),
      );
      ...
      while (session.completed < request.config.count) {
        ...
        for (const [i, row] of batch.entries()) {
          ...
          if (validateJson && !validateJson(row))
            throw new DomainError(
              'JSON_SCHEMA_VALIDATION',
              'Generated data violates the imported JSON Schema; adjust explicit generator settings',
            );
          const index = records.length;
          if (
            request.negative &&
            Math.floor((index + 1) * request.negative.rate) >
              Math.floor(index * request.negative.rate)
          ) {
            const negative = negativeCase(
              row,
              request.config,
              request.negative.field,
              request.negative.category,
              session.completed - batch.length + i,
            );
            records.push({
              ...negative.record,
              _testCase: {
                ...
                fieldPath: negative.fieldPath,
                expectedViolation: negative.expectedViolation,
                original: negative.original,
                isolated: negative.isolated,
                issues: negative.issues.map((item) => ({ ...item })),
              },
            });
        ...
        send({
          ...
          total: request.config.count,
        });
        await new Promise<void>((resolve) => setTimeout(resolve, 0));
      }
      manifest = session.manifest();
      manifest.sourceContentHashes = Object.fromEntries(
        await Promise.all(
          (request.config.pools ?? []).map(async (pool) => {
            const digest = new Uint8Array(
              await crypto.subtle.digest(
                'SHA-256',
                new TextEncoder().encode(JSON.stringify(pool.records)),
              ),
            );
            return [
              pool.source.id,
      ...
      report = request.negative
        ? 'Negative records include _testCase metadata and all actual validation issues.'
        : undefined;
    } else if (request.type === 'datasets') {
      const registry = createGeneratorRegistry(
        await loadTextProvider([
          request.config.locale,
          ...request.definitions.flatMap((definition) => locales(definition.schema.fields)),
        ]),
      );
      ...
      if (replacements.length && request.records.length) {
        if (!request.config)
          throw new DomainError(
            'TRANSFORM_CONFIG',
            'Synthetic replacement requires generation settings',
          );
        ...
        const config = {
          ...
          schema: {
            ...
            version: 1,
            fields: replacements.map((rule) => ({
              id: rule.field,
              name: rule.field,
              generator: rule.generator ?? 'text',
      ...
      report =
        'Local transformation complete. Masking and pseudonymization do not guarantee anonymity.';
    }
    send({
      type: 'result',
      jobId: request.jobId,
      preview: records.slice(0, 250),
      count: Object.keys(datasets).length
        ? Object.values(datasets).reduce((total, values) => total + values.length, 0)
        : records.length,
      manifest,
      datasets: Object.entries(datasets).map(([name, rows]) => ({ name, count: rows.length })),
      report,
    });
  } catch (error) {
    send({ type: 'error', jobId: request.jobId, message: errorMessage(error) });
  }
};
